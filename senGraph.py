...
class senGraph:
    ...
    def json(self, output='default.json', layout="layout",
             title='Visual', scale=[25., 15.],
...
class senDBSQL:
    def __init__(self, db=None, sql=None, table=None):
        ...
        if sql==None:
            import SQLite
            self.sql = SQLite.SQLite(db=(db==None and "/home/ron/inputdata/Ron/fullset/invpatC.upper.Jan2011.sqlite3" or db), tbl=self.table)
            self.sql.open()
        else:
            self.sql = sql
    ...
    def graph(self, vertex_list=None, where=None, output=None, flag=[]):
        ...
        self.sql.c.executescript("""
            DROP TABLE IF EXISTS vx0;
            DROP TABLE IF EXISTS ed0;
            CREATE INDEX IF NOT EXISTS G0_AY ON G0 (AppYearStr);
            CREATE INDEX IF NOT EXISTS G_id ON G0 (Patent);
            CREATE INDEX IF NOT EXISTS G_ed ON G0 (Invnum_N, Patent);
            ...
            CREATE TEMPORARY TABLE ed0 AS
                SELECT  a.Invnum_N AS h, b.Invnum_N AS t, a.AppYearStr AS AppYear, a.Patent AS Patent, a.Class AS Class
                  FROM  G0 AS a INNER JOIN G0 AS b
                    ON  a.Patent=b.Patent AND a.Invnum_N<b.Invnum_N;
            """)
