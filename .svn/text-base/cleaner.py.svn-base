...
#SET UP DATABASE - Prep a 'AsgNum' in the beginning
class orgClean():
    """
        Example Run:
            import orgClean
    ...
    def disambig(self):
        ...
        s.c.executescript("""
            ...
            """ % (fld, fld, fld, fld, fld, uqKey, uqKey, uqKey, other, table, fld, fld, fld, uqKey))
        ...
        if s.c.execute("SELECT count(*) FROM wrd").fetchone()[0]==0:
            ...
            def wordOrder(word):
                ...
                if len(wordStr)>5 and wordStr not in blankDct:
                    #if its a really general word.. don't use it for blocking
                    return wordStr
                else:
                    return word
        ...
        def adjMatch(numD=1, numS=1, jaro=0.90):
            while True:
                #ADJACENCY SEARCH -- IMPROVE THIS!
                aDct = {}
                allasg = s.c.execute("SELECT %s%d, %s2 FROM grp ORDER BY %s%d" % (fld, numD, uqKey, fld, numS)).fetchall()
                curr = ["", ""]
                for x in allasg:
                    if curr[1]!=x[1]:
                        jw = jarow(curr[0], x[0])
                        if jw > jaro:
                            x0, x1 = x[1]<curr[1] and [curr[1], x[1]] or [x[1], curr[1]]
                            aDct[x0] = x1
                        curr = x
                ...
                for x in aDct.keys():
                    s.c.execute("UPDATE grp SET %s2=? WHERE %s2=?" % (uqKey, uqKey), (aDct[x], x))

        print datetime.datetime.now(), "Adjacent Matching"
        adjMatch(numS=2)
        adjMatch(numS=3)
